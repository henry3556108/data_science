...
class Involution(nn.Module):
    def __init__(self, kernel_size, in_channel=4, stride=1, group=1,ratio=2):
        super().__init__()
        self.kernel_size=kernel_size
        self.in_channel=in_channel
        self.stride=stride
        self.group=group
        assert self.in_channel%group==0 # group 一定要能夠整除 channel 不然會炸錯
        self.group_channel=self.in_channel//group
        self.conv1=nn.Conv2d(
            in_channels=self.in_channel,
            out_channels=self.in_channel//ratio,
            kernel_size=1
        )
        self.bn=nn.BatchNorm2d(in_channel//ratio)
        self.relu=nn.ReLU()
        self.conv2=nn.Conv2d(
            in_channels=self.in_channel//ratio,
            out_channels=self.group*self.kernel_size*self.kernel_size,
            kernel_size=1
        )
...
if __name__ == '__main__':
    ...
    involution=Involution(kernel_size=3,in_channel=4,stride=2, ratio=4, group=2)
